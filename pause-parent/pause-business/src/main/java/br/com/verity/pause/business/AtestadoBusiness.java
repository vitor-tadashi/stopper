...
public class AtestadoBusiness {
	...
	public AtestadoBean salvar(AtestadoBean atestado) throws BusinessException {
		...
		ControleDiarioBean controleDiario = controleDiarioBusiness.obterPorDataIdFuncionario(atestado.getControleDiario().getData(),idFuncionario);
	...
	public List<AtestadoBean> listarPorIdFuncionarioEPeriodo(Integer idFuncionario, String[] periodo) {
		...
		List<AtestadoEntity> entities = atestadoDAO.findByPeriodoAndIdFuncionario(funcionario.getId(), periodos[0], periodos[1]);
		List<AtestadoBean> atestados = atestadoConverter.convertEntityToBean(entities);
		for(int i = 0; i < entities.size(); i++) {
			atestados.get(i).setTipoAtestado(tipoAtestadoConverter.convertEntityToBean(entities.get(i).getTipoAtestado()));
			atestados.get(i).setControleDiario(controleDiarioConverter.convertEntityToBean(entities.get(i).getControleDiario()));
		}

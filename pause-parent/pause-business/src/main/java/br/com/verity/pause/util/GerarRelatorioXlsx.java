	...
	public String relatorioFuncionarioPeriodo(List<ConsultaCompletaBean> consultaCompleta, FuncionarioBean funcionario, String de, String ate) {
		...
		try {
			...
			for(ConsultaCompletaBean consulta : consultaCompleta){
				...
				row.createCell(2).setCellValue((consulta.getSobreAvisoId() != null && consulta.getSobreAvisoId() != 0)?"S":"");
				...
				row.createCell(19).setCellValue((consulta.getAtestadoQuantidadeHora() != null)?consulta.getAtestadoQuantidadeHora(): 0);
				row.createCell(20).setCellValue((consulta.getControleDiarioHoraTotal() != null)?consulta.getControleDiarioHoraTotal(): 0);
				row.createCell(21).setCellFormula("IF(U"+(linha+1)+"+T"+(linha+1)+">8,U"+(linha+1)+"+T"+(linha+1)+"-8,0)");
				row.createCell(22).setCellFormula("IF(OR(B"+(linha+1)+"=\"Dom\", B"+(linha+1)+"=\"Sáb\"),0,IF(U"+(linha+1)+"+T"+(linha+1)+"<8,U"+(linha+1)+"+T"+(linha+1)+"-8,0))");
				row.createCell(23).setCellValue((consulta.getControleDiarioAdcNoturno() != null)?consulta.getControleDiarioAdcNoturno() : 0);
				row.createCell(24).setCellValue((consulta.getControleDiarioSA() != null)? consulta.getControleDiarioSA() : 0);
			...
			row.createCell(21).setCellFormula("IF(U"+(linha+1)+">0,TEXT(U"+(linha+1)+"/24,\"d hh:mm\"),TEXT(-(U"+(linha+1)+")/24,\"- d hh:mm\"))");
			linha++;
			...
			row = sheet.createRow(linha);
			row.createCell(0).setCellValue(verificarData.qualSemestre(consultaCompleta.get(0).getControleMensalMes()));
			...
			row = sheet.createRow(linha);
			row.createCell(0).setCellValue("Legenda : Origem Marcação: M- Manual / E- Eletrônico / SA = Sobre Aviso (Plantão); SA = Sobre Aviso sem Acionamento / ST = Sobre Aviso Trabalhado / TP = Tipo do Ponto");
			linha++;
